...
public final class DIContainer {
  typealias Method = (_ scope: DIContainer) -> Any

	public func resolve<T>(_: T.Type) throws -> T {
		return try resolver.resolve(self, type: T.self) { (initializer: Method) in return initializer(self) }
	}
	
	public func resolve<T>(_: T.Type, name: String) throws -> T {
		return try resolver.resolve(self, name: name, type: T.self) { (initializer: Method) in return initializer(self) }
	}
	
	public func resolveMany<T>(_: T.Type) throws -> [T] {
		return try resolver.resolveMany(self, type: T.self) { (initializer: Method) in return initializer(self) }
	}
	
	public func resolve<T>(_ object: T) throws {
		_ = try resolver.resolve(self, type: type(of: object)) { object }
	}
  ...
  internal func resolve(RType rType: RTypeFinal) throws -> Any {
    return try resolver.resolve(self, rType: rType) { (initializer: Method) in return initializer(self) }
  }
...
extension DIContainer {
  public func resolve<T>(byTypeOf obj: T) throws -> T {
    return try resolver.resolve(self, type: type(of: obj)) { (initializer: Method) in return initializer(self) }
  }
}
