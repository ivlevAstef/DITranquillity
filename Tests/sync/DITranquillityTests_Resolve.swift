...
class DITranquillityTests_Resolve: XCTestCase {
    ...
    func test01_ResolveByClass() {
        ...
        let service_fast: FooService = container.resolve()
    ...
    func test02_ResolveByProtocol() {
        ...
        let service_fast: ServiceProtocol = container.resolve()
        XCTAssertEqual(service_fast.foo(), "foo")
    }
    ...
    func test03_ResolveByClassAndProtocol() {
        ...
        let service_protocol: ServiceProtocol = container.resolve()
        XCTAssertEqual(service_protocol.foo(), "foo")
        
        let service_class: FooService = container.resolve()
    ...
    func test04_ResolveWithInitializerResolve() {
        ...
        let inject: Inject = container.resolve()
    ...
    func test05_ResolveWithDependencyResolveOpt() {
        ...
        let inject: InjectOpt = container.resolve()
        XCTAssertEqual(inject.service!.foo(), "foo")
    }
    ...
    func test07_ResolveWithDependencyResolveImplicitly() {
        ...
        let inject: InjectImplicitly = container.resolve()
        XCTAssertEqual(inject.service.foo(), "foo")
    }
    ...
    func test10_ResolveMultiplyLazyProviderByTag() {
        ...
        let valueFoo = serviceFoo.value.foo()
        XCTAssertEqual(valueFoo, "foo")
        
        let serviceBar: Provider<ServiceProtocol> = container.resolve(tag: BarService.self)
        let valueBar = serviceBar.value.foo()
        XCTAssertEqual(valueBar, "bar")
        
        let opaqueFoo = Unmanaged.passUnretained(serviceFoo.value as AnyObject).toOpaque()
        let opaqueBar = Unmanaged.passUnretained(serviceBar.value as AnyObject).toOpaque()
    ...
    func test11_ResolveMultiplySingleByTag() {
        ...
        let serviceFoo: ServiceProtocol = by(tag: FooService.self, on: container.resolve())
        XCTAssertEqual(serviceFoo.foo(), "foo")
    ...
    func test12_ResolveMultiplyLazyMany() {
        ...
        let services: [Lazy<ServiceProtocol>] = many(container.resolve())

        XCTAssertEqual(services.count, 2)
        let values = (services[0].value.foo(), services[1].value.foo())
    ...
    func test12_ResolveMultiplyProviderMany() {
        ...
        let services: [Provider<ServiceProtocol>] = many(container.resolve())
    ...
    func test13_ResolveCircular2() {
        ...
        let a: Circular2A = container.resolve()
        XCTAssert(a === a.b.a)
        XCTAssert(a.b === a.b.a.b)
        
        let b: Circular2B = container.resolve()
        XCTAssert(b === b.a.b)
    ...
    func test14_ResolveCircular3() {
        ...
        let a: Circular3A = container.resolve()
        XCTAssert(a === a.b.c.a)
        XCTAssert(a.b === a.b.c.a.b)
        XCTAssert(a.b.c === a.b.c.a.b.c)
        
        let b: Circular3B = container.resolve()
        XCTAssert(b === b.c.a.b)
        XCTAssert(b.c === b.c.a.b.c)
        XCTAssert(b.c.a === b.c.a.b.c.a)
        
        let c: Circular3C = container.resolve()
        XCTAssert(c === c.a.b.c)
    ...
    func test15_ResolveCircularDouble2A() {
        ...
        let a: CircularDouble2A = container.resolve()
        XCTAssert(a.b1 !== a.b2)
        XCTAssert(a === a.b1.a)
    ...
    func test15_ResolveCircularDouble2B() {
        ...
        let b: CircularDouble2B = container.resolve()
        XCTAssert(b === b.a.b1)
        XCTAssert(b === b.a.b2)
    ...
    func test17_DependencyIntoDependency() {
        ...
        let c: DependencyC = container.resolve()
    ...
    func test18_ResolveManyWithNamesTags() {
        ...
        let services: [ServiceProtocol] = many(container.resolve())
        XCTAssertEqual(services.count, 5)
    ...
    func test19_resolvePerContainer() {
        ...
        let service1_1: FooService = container1.resolve()
        let service1_2: FooService = container2.resolve()
        let service2_1: FooService = container1.resolve()
        let service2_2: FooService = container2.resolve()
    ...
    func test20_ResolveByTagAndTag() {
        ...
        let serviceFooTag1: ServiceProtocol? = by(tag: FooService.self, on: by(tag: BarService.self, on: container1.resolve()))
        XCTAssertEqual(serviceFooTag1?.foo() ?? "", "foo")
        
        let serviceFooTag2: ServiceProtocol? = by(tag: FooService.self, on: by(tag: BarService.self, on: container2.resolve()))
    ...
    func test20_ResolveLazyByTagAndTag() {
        ...
        let value1 = serviceFoo1.value?.foo() ?? ""
        XCTAssertEqual(value1, "foo")
        
        let serviceFoo2: Lazy<ServiceProtocol?> = container2.resolve(tag: FooService.self)
        let value2 = serviceFoo2.value?.foo() ?? ""
        XCTAssertEqual(value2, "foo")
        
        
        let serviceFooTag1: Lazy<ServiceProtocol?> = by(tag: FooService.self, on: by(tag: BarService.self, on: container1.resolve()))
        let tagValue1 = serviceFooTag1.value?.foo() ?? ""
        XCTAssertEqual(tagValue1, "foo")
        
        let serviceFooTag2: Lazy<ServiceProtocol?> = by(tag: FooService.self, on: by(tag: BarService.self, on: container2.resolve()))
        let tagValue2 = serviceFooTag2.value?.foo() ?? ""
    ...
    func test20_ResolveByTagAndTagShort() {
        ...
        let serviceFooTag1: ServiceProtocol? = by(tags: FooService.self, BarService.self, on: container1.resolve())
        XCTAssertEqual(serviceFooTag1?.foo() ?? "", "foo")
        
        let serviceFooTag2: ServiceProtocol? = by(tags: FooService.self, BarService.self, on: container2.resolve())
        XCTAssertEqual(serviceFooTag2?.foo() ?? "", "")
    }
    ...
    func test21_ResolveByTagAndMany() {
        ...
        let servicesByTag1: [ServiceProtocol] = many(by(tag: FooService.self, on: container.resolve()))
    ...
    func test21_ResolveLazyByTagAndMany() {
        ...
        XCTAssertEqual(services.count, 4)
        let valueFirst = services.first?.value.foo() ?? ""
        XCTAssertEqual(valueFirst, "foo")
        
        let servicesByTag1: [Lazy<ServiceProtocol>] = many(by(tag: FooService.self, on: container.resolve()))
        
        XCTAssertEqual(servicesByTag1.count, 2)
        let valueLast = servicesByTag1.last?.value.foo() ?? ""
    ...
    func test22_ResolveByTagTagAndMany() {
        ...
        let servicesByTag2: [ServiceProtocol] = many(by(tag: BarService.self, on: container.resolve()))
        XCTAssertEqual(servicesByTag2.count, 2)
        
        let servicesByTag3: [ServiceProtocol] = many(by(tag: BarService.self, on: by(tag: FooService.self, on: container.resolve())))
    ...
    func test22_ResolveByTagTagAndManyShort() {
        ...
        let servicesByTag3: [ServiceProtocol] = many(by(tags: BarService.self, FooService.self, on: container.resolve()))
        XCTAssertEqual(servicesByTag3.count, 1)
    }
    ...
    func test23_ResolveByTagTagTagShort() {
        ...
        let service: ServiceProtocol? = by(tags: FooService.self, BarService.self, ServiceProtocol.self, on: container.resolve())
        XCTAssertEqual(service?.foo() ?? "", "foo")
        
        let serviceNot: ServiceProtocol? = by(tags: FooService.self, BarService.self, Inject.self, on: container.resolve())
    ...
    func test24_ResolveUseKeyPathAndModificators() {
        ...
        let inject: ManyInject = container.resolve()
    ...
    func test25_PostInitCycle() {
        ...
        _ = container.resolve() as Circular2B
    ...
    func test26_many() {
        ...
        let test1: ManyTest = container.resolve()
        let test2: ManyInitTest = container.resolve()
