...
class DITranquillityTests_Build: XCTestCase {
  ...
  func test07_BiggerGraph() {
    ...
    container.register(F.init)
      .injection(\.c)
      .injection(\.d)
      .lifetime(.objectGraph)
    
    container.register(CL1.init)
      .injection(\.cl2)
      .lifetime(.objectGraph)
    
    container.register(CL2.init)
      .injection(\.cl3)
      .lifetime(.objectGraph)
    
    container.register(CL3.init)
      .injection(\.b)
      .lifetime(.objectGraph)
    
    XCTAssertFalse(container.validate())
